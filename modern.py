...
def place_image(where, posx, posy, image_path):
    ...
    image = customtkinter.CTkImage(light_image=light_image, dark_image=dark_image,size=(640,640))
    image_widget = customtkinter.CTkLabel(
        where,
...
menuframe = customtkinter.CTkFrame(master=root)
#menuframe.grid(row=0, pady=10, padx=0,sticky="nw")
file_menu = customtkinter.CTkOptionMenu(menuframe,corner_radius=0, values=["New", "Open", "Save", "Exit"])
file_menu.grid(row=0, column=0, pady=10, padx=10)
file_menu.set("File")
appearance_menu = customtkinter.CTkOptionMenu(menuframe,corner_radius=0, values=["Light", "Dark", "System"], command=change_appearance_mode_event)
...
subframe = customtkinter.CTkFrame(master=frame,corner_radius=90)

customtkinter.CTkLabel(master=subframe, text="Username",font=("ArialBlack",18)).pack(pady=10)
customtkinter.CTkLabel(master=subframe, text="Username",font=("ArialBlack",18)).pack(pady=10)
customtkinter.CTkLabel(master=subframe, text="Username",font=("ArialBlack",18)).pack(pady=10)
customtkinter.CTkLabel(master=subframe, text="Username",font=("ArialBlack",18)).pack(pady=10)
